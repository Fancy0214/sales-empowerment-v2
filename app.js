...
const scripts = [
    ...
    // === 异议拆解 ===
    { id: 10, category: "objection", title: "「我们已经有合作方了」", scene: "最常见的异议", content: "那挺好的，说明您英国方向有积累。我也不说让我们替代谁，就问您一个事：现在合作方，佣金月结还是季度结？周末出了问题找不找得到人？院校覆盖有没有缺口？多一个渠道不是坏事，关键时刻能兜底，而且我们佣金月结，不占用您的资金。", rating: 5, type: "异议拆解" },
    { id: 11, category: "objection", title: "「你们佣金没有XX高」", scene: "老板/销售对佣金敏感", content: "佣金比例确实重要，但您算过总账吗？我们是月结，不少同行是季度甚至半年才打款——等于您白白垫了几个月的资金。另外我们签约效率高，同样时间能多出几单，总收益其实更划算。还有个事，我们能垫付部分费用，这在行业里不多见，对您现金流帮助很大。", rating: 5, type: "异议拆解" },
    { id: 12, category: "objection", title: "「不确定你们服务行不行」", scene: "信任未建立-特别是后期老师", content: "这个担心太正常了，后期老师最怕对接出了问题找不到人。这样，您先推1-2个学生试试——我们承诺工作日2小时响应，周末也能找到人，申请进度实时可查。体验不好随时停，没有任何风险。27年专注英国不是白做的，后期这块我们有底气。", rating: 5, type: "异议拆解" },
    { id: 13, category: "objection", title: "「你们能做哪些院校？覆盖够不够？」", scene: "外联/销售关心院校资源", content: "好问题，英国前100有开放代理的院校我们基本能做——KCL、曼大、布里斯托、华威、格拉斯哥、利兹、南安普顿、谢菲、杜伦、诺丁汉这些都是我们的主力。爱尔兰TCD也有佣金。如果您有特别想做的院校，告诉我，我去确认能不能加进来。我们院校池一直在扩展。", rating: 5, type: "异议拆解" },
    { id: 14, category: "objection", title: "「佣金安全吗？会不会跑路？」", scene: "老板/个人代理最关心安全", content: "这个我能理解，27年不是白干的，我们佣金安全是底线。月结不拖欠，每个case的佣金明细清清楚楚。您要是不放心，我们可以先从小case开始，跑一单您就知道了。很多现在的大合作方，一开始也是这么试过来的。", rating: 4, type: "异议拆解" },
    { id: 15, category: "objection", title: "「我只做美加/澳新，不做英国」", scene: "业务方向不匹配", content: "了解，不过这两年不少原来只做美加的机构也开始加英国线了——英国申请周期短、出结果快，对现金流很友好。而且我们不需要您大投入，您只要有学生想申英国，我们就能接，您只管前端签约，后面全交给我们。哪天想试了，随时联系我。", rating: 3, type: "异议拆解" },
...
function generateScript() {
    ...
    // === 异议预判（留学行业三大异议：佣金/服务/院校） ===
    let objection = '';
    if (reachStage === '签约阶段' || reachStage === '意向确认阶段') {
        objection = `您可能还有几个顾虑，我逐个回应：\n💰 佣金方面：月结不拖欠，部分可垫付，佣金明细每笔清楚，27年没出过安全问题\n🔧 服务方面：工作日2小时响应，周末节假日有人，AI系统实时同步进度，紧急情况24小时电话\n🏫 院校方面：英国前100有代理的基本能做，如果有哪些特定院校您想做，告诉我，我去确认能不能加`;
    } else {
        objection = `如果您有任何顾虑，不外乎三种：\n• 担心佣金不安全？我们月结27年，零纠纷，可从小case试起\n• 担心服务跟不上？工作日2小时响应，周末也有人，先试一个case体验\n• 担心院校覆盖不够？英国前100有代理的基本能做，有特殊需求我去确认\n很多合作方都是从1-2个学生试起，发现靠谱才逐步扩大的。`;
    }
    scriptBlocks.push({ title: '异议预判', icon: 'fa-shield-alt', content: objection });
    
    // === 促单/下一步 ===
    let closing = '';
    if (reachStage === '签约阶段') {
        closing = `这样，我建议这周就把合作确认函签了，下周就能开始对接学生。佣金月结、服务2小时响应、院校前100覆盖——这些咱们都确认了，早启动早出结果。您看周四还是周五方便？`;
    } else if (reachStage === '意向确认阶段') {
        closing = `要不我们约个时间，我带一份详细的合作方案过来——院校清单、佣金政策、服务标准都列清楚，您看这周四或周五方便吗？`;
    } else if (reachStage === '维护阶段' || reachStage === '产出阶段') {
        closing = `后续我会定期跟您同步最新院校政策和佣金变化，有任何问题随时联系我。另外最近有几个新签约的院校，我整理好发您。`;
    } else {
        closing = `不如这样，我把最新的英国院校资源清单和佣金政策发给您，您先看看院校覆盖和佣金条件合不合适。有问题随时联系我，咱们约个时间详细聊。`;
    }
